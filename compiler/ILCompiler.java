...
{
	public static void compile(File input) throws FileNotFoundException
	{
	...
	private static void functionDeclarations(Line line)
	{
		...
		switch (line.getClass().getSimpleName())
		{
			...
			case "DefLine":
				...
				functionDeclarations(block);
				System.out.println("label _tl" + line.getID());
				List<String> params = ((DefLine) line).getParams();
				for (int i = params.size() - 1; i >= 0; i--)
					System.out.println("getparam " + params.get(i));
		...
		switch (line.getClass().getSimpleName())
		{
			...
			case "AssignmentLine":
				...
				for (int i = 0; i < assignValues.size(); i++)
					System.out.println("_" + i + "tl" + line.getID() + " = " + compile(assignValues.get(i)));
				for (int i = 0; i < assignValues.size(); i++)
				{
					...
					if (target instanceof ExpressionIdentifier)
					{
						...
						{
							System.out.println(((ExpressionIdentifier) target).getReference() + " = _" + i + "tl" + line.getID());
							continue;
						}
					}
			...
			case "PrintLine":
				...
				for (Expression expr : printLine.getPrintExpressions())
					System.out.println("param " + compile(expr));
				if (printLine.isPrintNewline())
				{
					System.out.println("_tl" + line.getID() + " = \"\\n\"");
					System.out.println("param _1tl" + line.getID());
					System.out.println("call print " + (printLine.getPrintExpressions().size() + 1));
				}
				else
					System.out.println("call print " + printLine.getPrintExpressions().size());
				System.out.println("returnvalue _2tl" + line.getID());
				break;
			case "BreakLine":
				throw new UnsupportedOperationException();
			case "ContinueLine":
			...
			case "IfLine":
				...
				for (int i = 0; i < conditions.size(); i++)
				{
					...
					System.out.println("jumpunless " + str + " _" + i + "tl" + line.getID());
					compile(blocks.get(i));
					System.out.println("jump _tl" + line.getID());
					System.out.println("label _" + i + "tl" + line.getID());
				}
			...
			case "WhileLine":
				...
				System.out.println("label _1tl" + line.getID());
				str = compile(condition);
				System.out.println("jumpunless " + str + " _2tl" + line.getID());
				compile(mainBlock);
				System.out.println("jump _1tl" + line.getID());
				System.out.println("label _2tl" + line.getID());
				compile(thenBlock);
				System.out.println("label _3tl" + line.getID());
				break;
			case "ForLine":
				throw new UnsupportedOperationException();
			case "DefLine":
				DefLine defLine = (DefLine) line;
				System.out.println("function " + defLine.getName() + " " + defLine.getID());
				break;
			case "ClassLine":
				throw new UnsupportedOperationException();
			case "CallLine":
		...
		switch (expr.getClass().getSimpleName())
		{
			case "ExpressionLiteral":
				...
				switch (v.getClass().getSimpleName())
				{
					case "NullValue":
						throw new UnsupportedOperationException();
					case "BooleanValue":
						System.out.println(str + " = " + ((BooleanValue) v).getValue());
						break;
					case "IntValue":
						System.out.println(str + " = " + ((IntValue) v).getValue());
						break;
					case "StringValue":
						System.out.println(str + " = " + ((StringValue) v).getValue());
						break;
					default:
						throw new UnsupportedOperationException();
				}
			...
			case "ExpressionIdentifier":
				...
				if (var.getParent() != null)
					...
					System.out.println("_te" + expr.getID() + " = " + var.getReference());
			...
			case "ExpressionCall":
				...
				if (call.getFunction() instanceof ExpressionOperator)
				{
					...
					if (exprs.size() == 1)
						System.out.println("_te" + expr.getID() + " = " + compile(call.getFunction()) + " " + compile(exprs.get(0)));
					else if (exprs.size() == 2)
						System.out.println("_te" + expr.getID() + " = " + compile(exprs.get(0)) + " " + compile(call.getFunction()) + " " + compile(exprs.get(1)));
					else
				...
				else
				{
					...
					System.out.println("call " + str + " " + call.getOperands().size());
					System.out.println("returnvalue _te" + expr.getID());
					return "_te" + expr.getID();
				}
			...
			case "ExpressionTernary":
				...
				System.out.println("unlessjump " + str + " _1te" + expr.getID());
				str = compile(ternary.getTrueCase());
				System.out.println("_te" + expr.getID() + " = " + str);
				System.out.println("jump _2te" + expr.getID());
				System.out.println("label _1te" + expr.getID());
				str = compile(ternary.getFalseCase());
				System.out.println("_te" + expr.getID() + " = " + str);
				System.out.println("label _2te" + expr.getID());
				return str;
			default:
				throw new UnsupportedOperationException();
		}
